...
static void setSymbol(IBusUnispimEngine *unispim_engine, gchar *symbol);
...
static void ibus_unispim_update_input_mode(IBusUnispimEngine *unispim_engine)
{
	...
	gchar* period;
	IBusProperty* prop;
	IBusProperty* title_prop = ibus_prop_list_get(unispim_engine->props, 0);
	...
	if (context.english_state == ENGLISH_STATE_NONE) {
		...
		ibus_property_set_symbol(title_prop, ibus_text_new_from_printf("%s%s", "英", period));
		ibus_engine_update_property((IBusEngine *) unispim_engine, title_prop);
		prop = ibus_prop_list_get(unispim_engine->props, 1);
		label = ibus_text_new_from_static_string("中文全拼");
		ibus_property_set_label(prop, label);
